...
All the data for today's exercise can be downloaded from [here](https://drive.google.com/open?id=115sQqs8LVqQmsKYRORTIAu3KZvUHczh0). Although we provide sources, you do not need to download the data from a given source. 
...
The pipe function is part of the package `dplyr` in the tidyverse, and is used to simply transform a tibble. A cheat sheet for the `dplyr` package can by found by clicking Help -> Cheatsheets -> Data Transformation with dplyr, or [at this link](https://www.rstudio.org/links/data_transformation_cheat_sheet).
...
```{r filterselect, warning=FALSE,message=FALSE,error=FALSE}
iris <- df %>% 
  filter(DEP=="75") %>%
